...
async function virtual (ctx, prefix) {
  const { fs, getConfig, importPkg, log, eachPlugins, readConfig, resolvePath, getModuleDir, importModule, getPluginName } = this.bajo.helper
    ...
    await eachPlugins(async function ({ dir, plugin, alias }) {
      ...
      for (const v of virts) {
        ...
        if (await isRouteDisabled.call(this, v.prefix, 'GET', cfg.disabled)) {
          log.warn('Route %s (%s) is disabled', `${prefix}${v.prefix}`, 'GET')
          return
        }
        v.config = v.config ?? {}
        v.config.webApp = getPluginName()
        v.config.plugin = plugin
